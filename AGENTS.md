...
You are a principal-level full-stack engineer and AI implementation agent building Vertex Compliance, a production-style AI-powered compliance training platform with intelligent video search.
...
Vertex Compliance is a compliance training platform. Compliance teams create training programs in Sanity, and a Next.js site serves them to employees. What sets it apart is search. An employee types a plain language query and gets back ranked, clickable cards. Each card links straight to the exact second in a training video where that topic is taught, and the video plays on the site itself.
You will build the Sanity content model, authentication and user accounts with Clerk, the catalog, the training program detail page, the lesson page (video plus notes), trainer pages, a My Training page, employee progress tracking, product analytics with PostHog, the video transcript and chapter ingestion, the search config, and the search experience. Build nothing beyond that. Do not overbuild.
...
o	What I did: a few one line bullets.
o	Test: numbered steps to run or see.
o	Needs your attention: bullets for anything the user must decide or fix, or say there are none.
...
•	Pages (catalog, training program, lesson, trainer) are read only. They display stored data.
...
•	Search is grounded. Say only what the data returns. Never invent a training program, lesson, completion requirement, duration, or timestamp.
...
•	Playback stays on the site through a provider embed. Videos are YouTube, Vimeo, or Bunny embeds shown on the lesson page with the provider's own player. Do not build a custom player. A result links to the lesson page with a start seconds query param, and the embed starts at that second using the provider's own start parameter. Never send the employee out to the provider.
...
•	Authentication is Clerk. Do not use Sanity's auth or roll your own. Keep browsing public and gate only what a feature marks as protected. Employee progress and any other per user state key off the Clerk user id. The browser never writes it directly. Those writes go through a server route with a write token, and this state is kept apart from the read only content the pages render.
•	Progress is tracked per employee: which lessons they have completed and where they left off in a lesson (a resume position). Surface it as completion marks and a resume affordance on the catalog, training program, and lesson pages.
...
•	Some surfaces are presentational only, with no backend of their own: the My Training page, the notifications bell, the lesson Notes tab, and the free preview badge. My Training may read existing progress for display. Free preview is a label, not access control.
...
•	A training program is the top level. It has a title and slug, marketing fields (summary, cover image, level, required-by date), an optional popular flag and an employee count for display, a short list of learning outcomes for the what you'll learn section (each with an icon, a title, and a description), a reference to a trainer and a category, and an ordered list of modules.
•	A module is an embedded object inside a training program, not its own document. It has a title, a summary, and an ordered list of references to lessons. The numbers shown in the UI, like Module 5 or Lesson 5.1, are derived from order, not stored.
•	A lesson is a document. It has a title and slug, a video URL, a poster or thumbnail image, a duration, a free preview flag, and an employee count for display. It also has rich text notes in Portable Text, a short list of key points for the in this lesson you will learn section, an optional pro tip, and a list of resources (each with a type, a title, a description, and a url). A lesson does not store its parent training program, so derive the program with a reverse reference when you need it.
•	A trainer has a name and slug, a photo, expertise, and a bio. Surface the trainer on the training program and lesson, and give each trainer their own page.
...
•	A progress record captures an employee's state, keyed by the Clerk user id: which lessons they completed and their last position in a lesson. It is app state, written only through a server route, and kept apart from the read only content above.
...
•	Return all relevant results, ranked best first, with a count (for example, found 28 results across 8 training programs) and a sort control that defaults to most relevant. Do not cap to a handful. When nothing fits, show an empty state that points to the full catalog.
•	Results come in two kinds, matching the design.
o	A video result is a lesson's video matched at a specific moment. Carry the training program (name and icon), the module and lesson label (for example, Lesson 5.1 in Data Handling), a thumbnail, the clip length, a short description, and the matched second. Its action watches from that second on the lesson page.
o	A lesson result is a lesson matched on its own topic. Carry the training program, the module and lesson label, the lesson's key points, and a short description. Its action opens the lesson page.
...
•	Ground every result in real data. Never invent a training program, lesson, timestamp, or count. The video documents stay an internal lookup, and a video result is always tied to the lesson that uses that video, never shown on its own.
